...
std::vector<float> IGraphNode::make_ticks(float lo, float hi, int target) {
    ...
    // ceil toward the first tick that is >= lo
    const float first = std::ceil(lo / step) * step;

    std::vector<float> ticks;
    ticks.reserve(static_cast<size_t>(target) + 2);

    // Allow a tiny overshoot tolerance to handle floating-point drift.
...
std::wstring IGraphNode::format_tick(float v, float step) {
    // Normalise -0 so we never render a spurious minus sign.
    if (v == 0.f) v = 0.f;

    // Derive required decimal places from the step magnitude.
    int decimals = 0;
    if (step < 1.f) {
        float s = step;
...
IGraphNode::Bounds IGraphNode::compute_bounds() const {
    ...
    if (auto_x || auto_y) {
        ...
        // No data or all-identical values — supply a sensible unit range.
...
void IGraphNode::draw_grid(
    ...
    // Grid base color — very dark, barely visible lines (demos aesthetic).
    const Color grid_col = attr.get_or<Color>(
        attribs::grid_color, Color(0.16f, 0.16f, 0.20f, 1.f));
    const float grid_w = attr.get_or<float>(attribs::grid_weight, 0.5f);
    ...
    // ── Horizontal grid lines at Y-tick positions ─────────────────────────
    {
        ...
        // Zero-line is a bit brighter to act as a visual anchor.
        ...
        for (float t : ticks) {
            // Screen Y — note the inversion: larger data values sit higher.
    ...
    // ── Vertical grid lines at X-tick positions — subtler than horizontal ─
    {
        const auto ticks = make_ticks(xl, xh, 6);

        // Vertical lines are deliberately fainter to keep the grid quiet.
...
void IGraphNode::draw_labels(
    ...
    const Color label_col = attr.get_or<Color>(
        attribs::label_color, Color(0.42f, 0.46f, 0.55f, 1.f));
    const float label_sz = attr.get_or<float>(attribs::label_font_size, 10.5f);

    const float xl = b.xl, xh = b.xh;
    const float yl = b.yl, yh = b.yh;

    // One format object reused for all labels; alignment is mutated between
    // the Y and X passes.
    ...
    // ── Y-axis labels ─────────────────────────────────────────────────────
    //
    // Right-aligned in the left margin, vertically centred on each tick line.
    //
    {
        ...
        const float label_right = px - 7.f;          // small gap before plot edge
        ...
        for (float t : ticks) {
            ...
            // Rect: x from node left edge to label_right, centred on yp.
            const Rect box{
                rect.x, yp - half_h,
                label_right - rect.x, half_h * 2.f
            };
    ...
    // ── X-axis labels ─────────────────────────────────────────────────────
    //
    // Centred horizontally below each tick position, just outside the plot.
    //
    {
...
void IGraphNode::draw_series(
    ...
    // Clip series lines to the plot rectangle so they never bleed into
    // the label margins.
    canvas.push_clip(Rect{ px, py, pw, ph });

    // Stroke style: round caps and joins give smoother polylines, especially
    ...
    for (const DataSeries& s : series) {
        ...
        if (n == 1) {
            // Single point — draw a small filled circle.
...
void IGraphNode::draw(Node& self, Canvas& canvas) {
    draw_default(canvas); // background fill + border via attr
    ...
    // Let any child nodes render on top (e.g. overlaid text annotations).
...
GraphNode::GraphNode(): Node(nullptr) {
    ...
    handle<IGraphNode>()->lp.share = 1.f; // fill parent by default
}
